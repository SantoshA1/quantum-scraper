...
return inputItems.map((item) => {
  ...
  const profileFloor =
    // QTP_MTF_SCALP_REWEIGHT_v6.2_20260521: long_term floor dropped for SCALP; scalp/swing floors unchanged.
    profile === 'SCALP' ? (scalpScore >= 60 && swingScore >= 45) :
    profile === 'SWING' ? (swingScore >= 60 && scalpScore >= 45) :
    (longTermScore >= 60 && swingScore >= 50);
  const pass = reasons.length === 0 && confluenceScore >= 65 && profileFloor;
