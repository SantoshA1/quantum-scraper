-- GATE_K_v2.2_K3_LOSS_ONLY_20260805 — PO-authorized 2026-08-05.
-- One predicate change vs v2.1: the K3 stop-out cooldown now requires net_pnl < 0.
-- Evidence: 08-04 09:55 K3 wrongly blocked an AKAM buy after a +$555 (+1.86R) WIN that
-- carried an H5-healed 'stop' label; the registry evidence for K3 (WDAY/ASML/CBRE) is
-- entirely about losses. Guard suite: tests/test-k3-cooldown.js (13/13).
-- Rollback: re-apply the v2.1 body (migration qet_kelly_gate_v2_1) — or remove the
-- net_pnl condition and restore gate_version.
...
BEGIN
  ...
  IF p_regime_mode <> 'off' AND v_direction IS NOT NULL THEN
    ...
    ELSE
      v_regime_info := jsonb_build_object('trend', v_regime.trend_regime,
        ...
        'mode', p_regime_mode);
      IF upper(v_regime.trend_regime) = 'UP' AND v_direction = 'bearish' THEN
        v_regime_violation := 'counter_regime_bearish_in_uptrend';
      ELSIF upper(v_regime.trend_regime) = 'DOWN' AND v_direction = 'bullish' THEN
  ...
  RETURN jsonb_build_object(
    ...
    'gate_version', 'GATE_K_v2.2_K3_LOSS_ONLY_20260805',
