-- GATE_K_v2.3.1_PIN_PROBATION_20260805 — Conclave ruling 2026-08-05, step 1 (emergency brake).
-- ONE functional delta vs v2.3: v_min_trades_eff := greatest(p_min_trades, 200) — Kelly may
-- not exit probation while the r_multiple column is uncertified (empty -1R loss bucket,
-- 92% uncertified writer provenance, winners labeled 'stop'; see
-- docs/CONCLAVE-BRIEF-KELLY-ACTIVATION-20260805.md). This is measurement_not_certified,
-- NOT negative_measured_edge — a broken gauge, not a judged-bad strategy. greatest() makes
-- the pin caller-proof (holds even if callers pass p_min_trades=40 explicitly).
-- Unpin: five-condition acceptance gate (loss mode at -1R; zero winner-'stop' rows; >=90%
-- certified lineage; R recomputed from fills, not relabeled; k* sign-stable under +/-20%
-- avg_loss_r perturbation), then re-apply v2.3 body (migration qtp_gate_k_v2_3_k1_regime_labels_20260805).
...
BEGIN
  ...
  -- ---- PIN_PROBATION_20260805 (Conclave ruling) --------------------------------
  -- Kelly may not engage while r_multiple is uncertified. Caller-proof floor.
  v_min_trades_eff := greatest(p_min_trades, 200);
  ...
  RETURN jsonb_build_object(
    ...
    'gate_version', 'GATE_K_v2.3.1_PIN_PROBATION_20260805',
