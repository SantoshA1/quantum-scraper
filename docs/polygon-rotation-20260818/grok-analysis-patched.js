...
try {
  ...
  if (_aKey && _aSec) {
    ...
    try {
      // gov 227b: key from n8n Variables by name; item._polygon_key fallback is dead once
      // Indicator Enrichment stops emitting it, kept only for deploy-order safety.
      const _pk = (typeof $vars !== 'undefined' && $vars && ($vars.POLYGON_API_KEY || $vars.POLYGON_KEY)) || item._polygon_key;
      if (_pk) {
...
try {
  // gov 227b: $vars-first, same fallback story as above.
  const _pkChart = (typeof $vars !== 'undefined' && $vars && ($vars.POLYGON_API_KEY || $vars.POLYGON_KEY)) || item._polygon_key;
  if (_pkChart) {
    const _cr = await qtpChartRead(this.helpers, ticker, _pkChart, (_mkt && _mkt._mkt_sector_etf) || null, API_KEY, QTP_ANTHROPIC_MODEL);
