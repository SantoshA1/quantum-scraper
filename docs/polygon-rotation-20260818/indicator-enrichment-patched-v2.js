...
// QTP-CHART-VISION-REALTIME — dynamic TradingView widget screenshot + image understanding.
// v4.6.1 (20260721, PO-directed): vision model -> claude-opus-4-8 (chart reads matter; cost accepted)
//   and chart interval 5min -> DAILY — the book is SWING now (multi-day holds, 5-9% targets);
//   a daily chart shows the structure the thesis lives on, not intraday noise. Prompt updated
//   for swing horizon; output field contract unchanged (chart_score/trend/pattern/confidence/
//   risk_flags/summary).
...
async function qtpChartVisionShadow(input) {
  const out = { ...(input || {}) };
  // v4.6.0 SMA-FIELD FIX: SM's 9/10 quality gate reads d.price_vs_sma50, which NOTHING ever
  // populated -> the SMA criterion failed on EVERY signal since inception. Computed here — every path calls this fn.
  try {
    const _pvs_p = parseFloat(out.price), _pvs_s = parseFloat(out.sma50);
    if (Number.isFinite(_pvs_p) && Number.isFinite(_pvs_s) && _pvs_s > 0) {
      out.price_vs_sma50 = _pvs_p >= _pvs_s ? 'ABOVE' : 'BELOW';
    }
  } catch (_pvse) {}
  try {
    ...
    if (!tickerClean) { out.chart_vision_enabled = false; out.chart_vision_status = 'SKIPPED_NO_TICKER'; out.chart_vision_call_mode = 'SKIPPED_NO_TICKER'; out.chart_vision_version = 'QTP_CHART_VISION_DAILY_v4.6.1_OPUS_20260721'; return out; }
    ...
    const chartEmbedUrl = `https://www.tradingview.com/widgetembed/?symbol=${encodeURIComponent(chartSymbol)}&interval=D&theme=dark&style=1&timezone=America%2FNew_York&withdateranges=1&hide_side_toolbar=0&allow_symbol_change=0&save_image=1`;
    ...
    out.chart_url_version = 'QTP_DYNAMIC_TRADINGVIEW_WIDGET_SCREENSHOT_URL_v4.2.20'; out.chart_vision_version = 'QTP_CHART_VISION_DAILY_v4.6.1_OPUS_20260721'; out.chart_vision_enabled = true; out.chart_vision_status = 'SCREENSHOT_URL_BUILT'; out.chart_vision_provider = 'anthropic_messages_image_understanding'; out.chart_screenshot_provider = 'image.thum.io_tradingview_widget';
    ...
    if (!_keyReal) { out.chart_vision_call_mode = 'SKIPPED_NO_ANTHROPIC_KEY_FAIL_OPEN'; out.chart_vision_status = 'SKIPPED_NO_ANTHROPIC_KEY'; return out; }
    const model = String((typeof $vars !== 'undefined' && $vars.ANTHROPIC_VISION_MODEL) || 'claude-opus-4-8').trim(); // v4.6.1 (PO-directed): vision default -> Opus 4.8; override via $vars.ANTHROPIC_VISION_MODEL
    out.chart_vision_model = model; out.chart_vision_call_mode = testVision ? 'TEST_FLAG_DAILY_WIDGET_SCREENSHOT_ANTHROPIC' : 'DAILY_WIDGET_SCREENSHOT_ANTHROPIC';
    const prompt = `Analyze this DAILY TradingView chart screenshot for ${chartSymbol}. This is for a SWING trade (multi-day hold, 5-9% target), so judge the multi-day structure: primary trend, key support/resistance levels near current price, recent pattern (base, breakout, breakdown, reversal, consolidation), and whether the current move has room to run or is extended into a level. Return strict JSON only with keys: chart_score number 0-100 (higher = cleaner swing setup in the direction of the recent move), trend BULLISH/BEARISH/NEUTRAL (the multi-day trend), pattern short string, confidence number 0-100, risk_flags array (e.g. major support below, overhead resistance, extended from mean, gap risk), summary one sentence. If chart is not visible, set pattern to NO_CHART_VISIBLE and confidence to 0. This is advisory enrichment only and must not assume execution.`;
    let response;
// v4.6.0: 2-attempt retry w/ 2s backoff (transient 5xx seen live). Outer catch keeps vision fail-open.
let _am = null; let _visErr = null; let _visAttempts = 0;
for (let _va = 1; _va <= 2; _va++) {
  _visAttempts = _va;
  try {
    _am = await this.helpers.httpRequest({ method: 'POST', url: 'https://api.anthropic.com/v1/messages', headers: { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01', 'Content-Type': 'application/json' }, body: { model, max_tokens: 512, messages: [{ role: 'user', content: [{ type: 'image', source: { type: 'url', url: screenshotUrl } }, { type: 'text', text: prompt }] }] }, json: true, timeout: 25000 });
    _visErr = null; break;
  } catch (_ve) {
    _visErr = _ve;
    if (_va < 2) { await new Promise(r => setTimeout(r, 2000)); }
  }
}
out.chart_vision_attempts = _visAttempts;
if (_visErr) throw _visErr;
...
out.chart_vision_api = 'anthropic_messages_primary';
    ...
    out.chart_vision_score = Number(parsed.chart_score ?? parsed.chart_vision_score ?? parsed.score ?? 0); out.chart_vision_trend = String(parsed.trend ?? parsed.chart_vision_trend ?? 'UNKNOWN'); out.chart_vision_pattern = String(parsed.pattern ?? parsed.chart_vision_pattern ?? 'UNKNOWN'); out.chart_vision_confidence = Number(parsed.confidence ?? parsed.chart_vision_confidence ?? 0); out.chart_vision_risk_flags = Array.isArray(parsed.risk_flags) ? parsed.risk_flags.join('; ') : String(parsed.risk_flags || parsed.chart_vision_red_flags || ''); out.chart_vision_summary = String(parsed.summary || parsed.chart_vision_summary || '').slice(0, 800); out.chart_vision_status = 'ANALYZED_DAILY_WIDGET_SCREENSHOT'; out.chart_vision_completed_at = new Date().toISOString();
  } catch (e) { out.chart_vision_status = 'ERROR_FAIL_OPEN'; out.chart_vision_error = String((e && e.response && e.response.body && JSON.stringify(e.response.body)) || (e && e.response && e.response.data && JSON.stringify(e.response.data)) || (e && e.message) || e).slice(0, 900); out.chart_vision_fail_open_alert = true; out.chart_vision_call_mode = out.chart_vision_call_mode || 'DAILY_WIDGET_SCREENSHOT_ANTHROPIC_ERROR_FAIL_OPEN'; out.chart_vision_version = 'QTP_CHART_VISION_DAILY_v4.6.1_OPUS_20260721'; }
  return out;
}

const _creds_IE = ($getWorkflowStaticData('global')._credentials) || {};
// gov 227 (2026-08-18) Polygon key rotation: read n8n Variables FIRST so rotating the key is
// one Variables edit; the staticData fallback keeps today's path alive until the Variable is
// seeded — provably no behavior change while $vars.POLYGON_API_KEY is absent. SE-C1 stays
// fail-closed: no key from either source still halts enrichment rather than trading blind.
let _pk_vars = null;
try { if (typeof $vars !== 'undefined' && $vars) { _pk_vars = $vars.POLYGON_API_KEY || $vars.POLYGON_KEY || null; } } catch (e) {}
const POLYGON_KEY = (_pk_vars && String(_pk_vars).trim()) || _creds_IE.polygon_key;
if (!POLYGON_KEY) throw new Error('SE-C1: polygon_key missing from $vars.POLYGON_API_KEY and staticData._credentials');
...
// gov 227b (2026-08-18): the Fix #17 passthrough (`enriched._polygon_key = POLYGON_KEY`) is
// REMOVED. It rode the key on every signal item into n8n execution logs — the leak that made
// the old key rotation-worthy would have re-exposed the NEW key within a day. The four
// downstream Polygon HTTP nodes and Grok AI Analysis now read $vars.POLYGON_API_KEY directly.
