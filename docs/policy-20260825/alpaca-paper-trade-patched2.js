// Alpaca Paper Trade v4.9.1 (Conclave S3 stop-regime telemetry — 2026-08-11) — EX-C1/C2/C3 execution fix (2026-08-10)
//
// ═══ v4.9.1, 2026-08-11 — ADDITIVE ONLY. No behaviour change. ═══
// The Conclave's 2026-08-11 ruling (S3) requires that the entry STOP REGIME be separable in
// every future edge computation, the way E3's exec_regime already makes the execution regime
// separable. Gate-K's sample spans three regimes — (a) wide stop + market orders, (b) 1.2%
// stop + market orders, (c) 1.2% stop + capped limits — and the 20-trade long-book bar will
// be met entirely under (c). Without this, the clean week re-contaminates the very sample it
// exists to clean.
// This node contributes the three facts only IT knows: the clamp constant in force, whether
// the clamp actually bound on this trade, and the raw ATR stop distance it overrode.
// QET Ledger H3 SQL v3 assembles them into sizing_meta.stop_regime.
// Nothing here is read by any decision path.
//
...
// gov241 Patch C (2026-08-26): EX-C3 geometry now follows the ratified 2.5% policy stop.
...
// QTP_EXIT_POLICY_v1_gov241_20260825 (Conclave 2026-08-25, Decision 1): FIXED 2.5% stop.
...
// QTP_EXIT_POLICY_v1_gov241_20260825: the measured policy has NO profit target — winners
...
if (TRAIL_PCT) {
  ...
  if (_exCapActive && _exVolFilled <= 0) {
    ...
    return [{ json: { ...prev,
      ...
      alpaca_signal_price: signal_price, alpaca_fresh_price: fresh_price, alpaca_slip_pct: slip_pct,
      alpaca_anchor_used: anchor_source, alpaca_exec_regime: _exRegime, alpaca_exec_cap_pct: _exCapPct * 100,
      alpaca_is_volatile: true, alpaca_bracket_v: '4.9.2' } }];
  }
  ...
  return [{ json: {
    ...
    alpaca_atr_used:    atr,
    // v4.9.1 Conclave S3 — additive stop-regime telemetry, read by nothing
    alpaca_entry_stop_clamp_pct: MAX_ENTRY_STOP_PCT * 100,
    alpaca_stop_clamped:         _stopDist < _rawStopDist,
    alpaca_raw_stop_dist:        r2(_rawStopDist),
    alpaca_signal_price: signal_price,
    alpaca_fresh_price:  fresh_price,
    alpaca_slip_pct:     slip_pct,
    alpaca_anchor_used:  anchor_source,
  ...
  const bracketBody = {
    ...
    // v4.9.2 (gov 211): TIF gtc. It was 'day' in v4.9/v4.9.1 (IOC emulation) — but bracket
    // TIF applies to ALL legs, so Alpaca cancelled the protective stop children of WMB and
    // WMT at the 2026-08-12 close and left both positions naked overnight (P0). The IOC
    // semantic for the ENTRY remains EX-C2's poll-then-cancel below; on the cancel-FAILED
    // path a gtc order now persists past the close and must be cancelled by hand — status
    // SKIPPED_NO_FILL_CANCEL_FAILED already pages a human for exactly that.
    type:            _exCapActive ? 'limit' : (__aptIsExt ? 'limit' : 'market'),
    time_in_force:   __aptIsExt ? 'day' : 'gtc',   // v4.9.2 gov211: day-TIF cancelled WMB/WMT stop children at the 08-12 close (P0) — bracket children must outlive the session; entry no-fill is EX-C2's poll-cancel, not TIF
    ...( _exCapActive
  ...
  if (_exCapActive && _exBr.outcome === 'UNREADABLE') {
    ...
    return [{ json: { ...prev,
      ...
      alpaca_signal_price: signal_price, alpaca_fresh_price: fresh_price, alpaca_slip_pct: slip_pct,
      alpaca_anchor_used: anchor_source, alpaca_exec_regime: _exRegime, alpaca_exec_cap_pct: _exCapPct * 100,
      alpaca_is_volatile: false, alpaca_bracket_v: '4.9.2' } }];
  }
  ...
  // outcome on the merits: any further fill is still bounded by the same capped limit, and
  // Alpaca scales the protective legs to the filled quantity. v4.9.2: under gtc a partial
  // remainder can persist past the close and fill later at a stale cap — rare, and the
  // 10:15 daily check flags any prior-day partial still working.
  ...
  if (_exPartial) {
    console.warn('[APT v4.9 EX-C2] ' + ticker + ' PARTIAL ' + _exBrFilled + '/' + qty + ' at cap — NOT cancelling: cancelling a bracket cancels the whole group and would strip protection from the filled shares. Remainder left working (TIF gtc, same capped limit).');
  }
