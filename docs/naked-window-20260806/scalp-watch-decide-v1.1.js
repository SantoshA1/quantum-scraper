...
// QTP Scalp Exit Watcher — PAPER ONLY v1.1
// v1.1 SCOPE FIX (gov 191, PO order 2026-08-06): this watcher closed BOTH of today's
// main-pipeline swing entries minutes after fill (WRB 13:46Z SCALP_MAX_ADVERSE_MOVE
// -$143.60; APA 14:30Z UNPROTECTED_OR_STOP_MISSING -$47.68 after the TSM recovery went
// naked). Swing positions belong to the TSM, not the scalp ruleset. v1.1 attributes every
// position by its entry order client_order_id (from the context query):
//   'qet-' prefix  -> main pipeline (F-DURABLE stamp)  -> SKIP_MAIN_PIPELINE_SCOPE
//   missing/unknown -> cannot prove it is a scalp       -> SKIP_UNATTRIBUTED_NOT_SCALP
// Only positively-attributed non-main entries remain closeable. Mirror: lib/scalp/scope.js
...
for (const r of rows) {
  ...
  const entryCoid = txt(r.entry_client_order_id);
  const isMainPipeline = entryCoid.startsWith('qet-');
  const isAttributedScalp = entryCoid !== '' && !isMainPipeline;
  ...
  else if (isMainPipeline) action = 'SKIP_MAIN_PIPELINE_SCOPE';
  else if (!isAttributedScalp) action = 'SKIP_UNATTRIBUTED_NOT_SCALP';
  ...
  const decision = {
    ...
    entry_client_order_id: entryCoid || null,
    scope_main_pipeline: isMainPipeline,
    scope_attributed_scalp: isAttributedScalp,
    action,
    reasons,
