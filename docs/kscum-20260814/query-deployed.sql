=WITH cfg AS (
  ...
  SELECT coalesce(sum(net_pnl), 0)::numeric AS cohort_cum_net, count(*) AS cohort_trades
  FROM quantum.v_expansion_cohort_pnl
), trip AS (
  SELECT
    ...
    ((SELECT cohort_cum_net FROM cum) <= (SELECT live_value FROM cfg WHERE constant_name = 'killswitch_cohort_cumulative_usd')) AS cum_trip
), ins AS (
