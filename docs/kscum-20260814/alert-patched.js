...
if ((tripped || blind) && t) {
  let text;
  if (j.cum_trip === true) {
    text = '🛑🛑 EXPANSION CUMULATIVE HALT — cohort NET ' + j.cohort_cum_net + ' USD over ' + j.cohort_trades + ' trades breached the ratified stop ' + j.cum_threshold_usd + ' USD (baseline: gov-214 re-enable). New entries paused 30d; Conclave RECONVENE required before resume.';
  } else if (tripped) {
