...
// QTP_SCANNER_UNIVERSE_SELFHEAL_v1_gov218_20260814 — the 2026-07-23 universe collapse.
//   The 06-30 change raised the batch 125 -> 600 so that "every name is evaluated every
//   5min run". That silently stopped being true on 2026-07-23: 85 tickers were added to
//   quantum.quantum_watchlist_raw on 07-22, the effective universe reached 622, and
//   622 > 600 re-engaged the rotation as a DEGENERATE 600 + 22 split. The 600-name head
//   then ran only every other cycle, and MAX_NEW_ENTRIES_PER_CYCLE's break exhausted the
//   quota near index ~208, while the 22-name tail got a whole cycle to itself.
//   Measured over the 16 sessions that followed: distinct symbols signalled 511 -> 83,
//   head coverage 82% -> 10.7%, and 391 of 622 tickers — every name from ES to WM,
//   including NVDA/MSFT/META/GOOGL/TSLA/JPM/LLY/V/MA — were not scanned ONCE.
//   Nothing alarmed, because nothing asserted coverage.
// FIX: the batch is DERIVED from the universe instead of being a constant the universe
//   can outgrow, and batches are EVEN — ceil(N / batchCount) — so a remainder can never
//   again become a starved micro-batch. While N <= SCANNER_MAX_PER_CYCLE the whole
//   universe is scanned every cycle, which is the 06-30 intent now enforced by
//   construction rather than by a number someone has to remember to raise.
//   SCANNER_MAX_PER_CYCLE is a RUNTIME ceiling (the 60s Code-node limit), never a cap on
//   what is tradeable: exceed it and the universe splits evenly and completes in
//   batchCount cycles, covering every name.
const SCANNER_MAX_PER_CYCLE = 1000;
const SCANNER_BATCH_COUNT = Math.max(1, Math.ceil(FULL_WATCHLIST_COUNT / SCANNER_MAX_PER_CYCLE));
const SCANNER_BATCH_SIZE = Math.max(1, Math.ceil(FULL_WATCHLIST_COUNT / SCANNER_BATCH_COUNT));
if (SCANNER_BATCH_COUNT === 1 || !Number.isFinite(state.quantumWatchlistBatchOffset) ||
    state.quantumWatchlistBatchOffset < 0 || state.quantumWatchlistBatchOffset >= FULL_WATCHLIST_COUNT) {
...
tickers = batch;
// QTP_SCANNER_ENTRY_CURSOR_v1_gov224_20260817 — the alphabetical starvation.
//   The evaluate loop below walks `tickers` from index 0 and BREAKS at
//   MAX_NEW_ENTRIES_PER_CYCLE signals. The only thing that ever moved its starting point was
//   the batch rotation above. gov 218 made the batch whole (624 <= 1000 => ONE batch, offset
//   pinned to 0), so from 2026-08-14 the walk restarted at index 0 EVERY cycle and found its
//   4 signals inside the A's. Measured 2026-08-17: 35 distinct symbols of 624 in a full
//   session, first letters {A,B} only — NARROWER than the collapse gov 218 fixed (that era
//   alternated offset 0/600, which is why it showed A + W..Z). gov 218 made every name
//   reachable and simultaneously removed the mechanism that reached them.
// FIX: a resume cursor. The walk starts where it stopped last cycle and wraps, so successive
//   cycles sweep the whole universe. MAX_NEW_ENTRIES_PER_CYCLE is UNCHANGED — it is a risk
//   limit on how much may be OPENED per cycle, and nothing here touches it. Rotation applies
//   only when the universe fits in one batch; with >1 batch the batch offset already advances
//   the walk, so the cursor stands down rather than fighting it.
...
state.quantumScanEntryCursorStart = _entryCursor;
state.quantumScanEntryCursorActive = _cursorActive;
state.quantumScanUniverseCount = FULL_WATCHLIST_COUNT;
state.quantumScanBatchSize = tickers.length;
state.quantumScanCoveragePct = FULL_WATCHLIST_COUNT ? Math.round(1000 * tickers.length / FULL_WATCHLIST_COUNT) / 10 : 0;
console.log('[SCAN] #' + state.scanCount + ' | Quantum Watchlist batch ' + batchStart + '-' + (batchEnd - 1) + ' of ' + FULL_WATCHLIST_COUNT + ' | processing ' + tickers.length + ' tickers (' + state.quantumScanCoveragePct + '% of universe, ' + SCANNER_BATCH_COUNT + ' batch(es) per full sweep) | nextOffset=' + state.quantumWatchlistBatchOffset + ' | ' + etHour + ':' + etMin + ' ET');
...
let _walked = 0;  // gov 224: positions consumed this cycle, for the resume cursor
...
// gov 224: resume where this cycle stopped, wrapping. A full sweep with no break lands on 0,
// which is correct — the next cycle starts a fresh pass over the whole universe.
state.quantumScanWalkedThisCycle = _walked;
state.quantumScanEntryCursor = (_cursorActive && tickers.length)
  ? (state.quantumScanEntryCursorStart + _walked) % tickers.length
  : 0;
console.log('[DONE] v3.2 | ' + tickers.length + ' tickers | ' + signals.length + ' signals (max ' + MAX_NEW_ENTRIES_PER_CYCLE + '/cycle) | Filtered: ' + filteredVolume + ' low-vol, ' + filteredDeviation + ' extended | Scan #' + state.scanCount + ' | cursor ' + state.quantumScanEntryCursorStart + ' -> ' + state.quantumScanEntryCursor + ' (walked ' + _walked + ' of ' + tickers.length + ', first=' + (tickers[0] || 'n/a') + ')');
return signals;
