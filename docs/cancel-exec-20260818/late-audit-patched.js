...
const LATE_AUDIT_VERSION = 'QTP_LATE_EXEC_FLOW_AUDIT_BUILDER_v4.1_CANCEL_IS_NOT_EXECUTION_20260818';
...
// v4.1 gov 226 (2026-08-18): an order that was never placed, or was cancelled unfilled, is NOT
// an execution. Before this, SKIPPED / SKIPPED_NO_FILL_WITHIN_CAP / BLOCKED_* items rode the
// PASS path into the finalize CASE's ELSE and were stamped EXECUTED — 15 modern-era rows, and
// every one burned a slot in the Gate-K wrapper's exec_today/sell_today daily caps (ECL
// 2026-08-18 09:55, TE-C4 skips 08-12/08-13, WMB/AMP no-fill cancels 08-11). A cancelled entry
// even carries an alpaca_order_id, so order-id presence proves nothing about execution.
// EXECUTED now requires a broker order outcome that is not a skip/block/ext-hours refusal.
const apStatusStr = String(alpaca_status || '');
const orderNotPlaced = /^(SKIPPED|BLOCKED|EXT_HOURS)/.test(apStatusStr);
const notPlacedStage = !orderNotPlaced ? null
  : (apStatusStr === 'SKIPPED_NO_FILL_WITHIN_CAP' ? 'ENTRY_CANCELLED_NO_FILL'
  : (/^SKIPPED/.test(apStatusStr) ? 'ENTRY_SKIPPED_EXECUTOR' : 'ENTRY_BLOCKED_EXECUTOR'));
...
const blockedStageLate = (() => {
  if (notPlacedStage) return notPlacedStage; // v4.1: a definitive executor refusal outranks bias-field presence
...
const final_outcome = orderNotPlaced
  ? 'ORDER_NOT_PLACED_OR_CANCELLED' // v4.1: ECL's cancel had an order id and read FILLED_OR_NEW
  : (alpaca_order_id
      ? 'FILLED_OR_NEW'
...
SET gate_decision                  = gate_decision || '|${lateAuditToken}',
    blocked_stage                  = ${orderNotPlaced
      ? `CASE WHEN COALESCE(blocked_stage,'') IN ('','NONE','UNKNOWN') THEN ${sqlText(notPlacedStage)} ELSE blocked_stage END`
      : `COALESCE(blocked_stage, ${sqlText(blockedStageLate)})`},
    ...
    audit_status                   = (
      CASE
        ...
        -- v4.1 gov 226: a skip/block/cancel at the executor is never an execution, regardless of
        -- what blocked_stage already says. Literal is JS-interpolated from alpaca_status.
        WHEN ${orderNotPlaced ? 'TRUE' : 'FALSE'} THEN 'REJECTED'
        ELSE 'EXECUTED'
      END
