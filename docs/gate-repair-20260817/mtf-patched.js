...
const scored = items.map(item => {
  ...
  // QTP_MTF_DEAD_INPUT_ABSTAIN_v1_gov221_20260817 — PO-authorized 2026-08-17.
  //   MTF_Bull/MTF_Bear have read constant 0 on EVERY signal since 2026-06-01, so the
  //   confluence engine emits mtf_confluence_score=0, mtf_tiers={} and
  //   final_mtf_confluence_decision='N/A'. mtfWouldBlock reads that as a FAILED gate.
  //   Its sibling _mtfFloorBlock already guards the identical case with `mtfScore > 0`
  //   — this node ALREADY knows a zero score means "no reading". Only this one branch
  //   forgot, and it has been vetoing on a dead sensor for 11 weeks: 10 kills in the
  //   fortnight to 2026-08-17, 9 of them longs. BMNR (bias 97, the strongest long of
  //   08-17) died here at 09:41 on score 0 with an empty tier set and decision 'N/A'.
  //   A dead sensor must ABSTAIN, not veto. This is deliberately fail-OPEN and is the
  //   opposite direction to gov 215's fail-closed config guard — the difference is that
  //   gov 215 guarded a RISK LIMIT (missing => stop trading) while this guards a
  //   SIGNAL-QUALITY opinion (missing => no opinion). The veto is restored in full the
  //   instant either score goes non-zero; nothing about a live MTF reading changes.
  const _mtfInputDead = !(Number.isFinite(mtfScore) && mtfScore > 0)
                     && !(Number.isFinite(aiMtfScore) && aiMtfScore > 0);
  output._mtf_input_dead = _mtfInputDead;
