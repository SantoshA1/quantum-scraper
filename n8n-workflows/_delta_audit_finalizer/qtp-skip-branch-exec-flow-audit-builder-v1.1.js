...
// v1.1 (AUDIT_STATUS_FINALIZE 20260720): SKIP-branch rows never reach the Late Audit
//   finalizer, so audit_status stayed PENDING despite a bound kill (incl. TICKER_BLOCKLIST).
//   A SKIP-branch entry is by definition NOT a fresh execution -> write audit_status=REJECTED.
...
(ts, symbol, side, vc_live_legacy, vc_live_v2, vc_shadow, vc_delta, gate_decision, parser_version,
 ...
 signal_id, idempotency_key, gate_lineage, kill_stage_attribution, audit_status)
SELECT
  CURRENT_TIMESTAMP,
  ${esc(symbol)},
  ${esc(side)},
