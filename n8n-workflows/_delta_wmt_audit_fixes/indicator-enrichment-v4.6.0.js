...
async function qtpChartVisionShadow(input) {
  ...
  // populated -> the SMA criterion failed on EVERY signal since inception (WMT 07-21 audit:
  // "Price not below SMA50" while 110.44 < 118.89). Compute it here — every path calls this fn.
  try {
  ...
  try {
    ...
    const model = String((typeof $vars !== 'undefined' && $vars.ANTHROPIC_VISION_MODEL) || 'claude-sonnet-5').trim(); // v4.6.0: vision default -> Sonnet 5 (advisory chart-read; ~5x cheaper/faster than Opus; flip back anytime via $vars.ANTHROPIC_VISION_MODEL) out.chart_vision_model = model; out.chart_vision_call_mode = testVision ? 'TEST_FLAG_REALTIME_WIDGET_SCREENSHOT_ANTHROPIC' : 'REALTIME_WIDGET_SCREENSHOT_ANTHROPIC';
    ...
    let response;
// v4.6.0: 2-attempt retry w/ 2s backoff (transient 5xx seen live: WMT 07-21 got a single 500 and went blind). Outer catch keeps vision fail-open.
