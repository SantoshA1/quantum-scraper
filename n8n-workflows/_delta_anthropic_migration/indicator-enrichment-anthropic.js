...
async function qtpChartVisionShadow(input) {
  ...
  try {
    ...
    const model = String((typeof $vars !== 'undefined' && ($vars.ANTHROPIC_VISION_MODEL || $vars.ANTHROPIC_MODEL)) || 'claude-opus-4-8').trim(); out.chart_vision_model = model; out.chart_vision_call_mode = testVision ? 'TEST_FLAG_REALTIME_WIDGET_SCREENSHOT_ANTHROPIC' : 'REALTIME_WIDGET_SCREENSHOT_ANTHROPIC';
