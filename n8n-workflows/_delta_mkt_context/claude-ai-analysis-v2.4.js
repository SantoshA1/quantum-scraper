...
// Opus reads it, fields land on the existing chart_vision_* contract. Fail-open. Any ticker;
// only analyzed (post-Bias/SSM) signals pay. Harness-verified execs 435818/435871.
...
async function qtpChartRead(helpers, tkr, pk, secEtf, anthKey, anthModel) {
  ...
  try {
    ...
    // [full computation block — session character (NR7/inside/close-location), MA stack + slopes,
    //  ADX14 Wilder, RSI14 Wilder + divergence, MACD 12/26/9 + cross recency, swing sequences
    //  (raw last-3 highs/lows, model classifies), S/R in ATR distances from swings + open-gap edges,
    //  20/60/120d ranges, unfilled gaps, RVOL + up/down volume ratio, RS vs sector ETF bars,
    //  ex-div lookup, earnings attempt (fail-open to UNKNOWN flag), narrative assembly,
    //  Opus strict-JSON call, mapping to chart_vision_* + chart_key_support/resistance/chart_swing_room]
    // AUTHORITATIVE SOURCE: live n8n vaqfCaELhOEWnkdo version a61428db-cebc-47d5-af77-1e0ab94506e5
    // (gov #138). Identical logic harness-verified in TEMP wf q26tts9zuikQiLJu exec 435871.
