...
// QTP_VC_R32_CONTRA_BOTH_v1_20260721: R3.2 hard-kill now requires BOTH legs (options AND dark pool)
// opposed — mirrors RCF v4. Single-leg opposition = red flag + cap 6, NOT auto-KILL. Evidence: the
// options-regime classifier's fade thesis (+GEX+greed->CONTRARIAN_SHORT) opposes momentum BUYs by
// construction; single-leg auto-KILL executed the weak side and killed the strong one (BUY alpha ~6x).
...
Rules:
R3.2 (hard-opposite-kill — CONTRA_BOTH policy, v1 20260721):
  The OPTIONS FLOW regime and DARK POOL regime represent informed money. The kill
  is structural ONLY when BOTH independent legs point against the execution.
  A single opposing leg is a caution, not a veto — the options-regime label is a
  contrarian/fade model and opposes momentum entries by construction, so it must
  be CONFIRMED by dark pool before it can kill.

  HOW TO READ THE DATA: look at the Regime value under "=== OPTIONS FLOW ==="
  (OPTIONS_REGIME) and under "=== DARK POOL ===" (DP_REGIME). Substring matching,
  case-insensitive (MODERATE_DISTRIBUTION matches "DISTRIBUTION").

  Opposing-leg definitions:
  For BUY / LONG execution:
    - OPTIONS leg opposes if OPTIONS_REGIME contains any of:
        CONTRARIAN_SHORT, GAMMA_SQUEEZE_DOWN, BEARISH, PUT_HEAVY,
        DISTRIBUTION, MODERATE_DISTRIBUTION
    - DARK POOL leg opposes if DP_REGIME contains any of:
  ...
  Decision rule:
  - BOTH legs oppose -> score = 0, final_verdict = KILL, cite "R3.2 (BOTH: OPTIONS_REGIME=..., DP_REGIME=...)".
    No exceptions — not for high bias, volume, RSI, cross-asset, or backtest.
  - EXACTLY ONE leg opposes -> do NOT auto-kill. Add a red flag citing
    "R3.2-SINGLE (OPTIONS_REGIME=... or DP_REGIME=...)" and CAP the score at 6
    (REJECT band) unless at least two independent confirmations align with the
    trade (daily trend, volume ratio >= 1.5x, cross-asset CONFIRMED, or clean
    chart trend), in which case cap at 7 (WEAK).
  - NEITHER leg opposes -> R3.2 does not apply.
...
R7 — HIGH-SCORE SAFETY FLOORS (no PASS-at-10 with structural problems):
  ...
  R7.1 (bias-floor-for-pass):
    bias_score ≥ 55. If bias_score < 55: cap at 7. Cite R7.1.
  R7.2 (volume-floor-for-pass):
    volume_ratio ≥ 1.0 (or BROAD_SCANNER exempt). If volume_ratio < 1.0 and
    alert_type ≠ BROAD_SCANNER: cap at 7. Cite R7.2.
  R7.3 (no-hard-opposite-for-pass):
    R3.2 BOTH-leg kill must not have fired (if it fired, score is 0 per R3.2).
    An R3.2-SINGLE flag also blocks PASS (its cap is 6-7 per R3.2).
...
R1.2 (profit-factor-ladder):
  Apply as a HARD CAP:
    PF < 0.5:        cap score at 1.    Cite R1.2.
    0.5 ≤ PF < 0.7:  cap score at 2.    Cite R1.2.
    0.7 ≤ PF < 0.9:  cap score at 3.    Cite R1.2.
    0.9 ≤ PF < 1.0:  cap score at 4.    Cite R1.2.
    1.0 ≤ PF < 1.1:  cap score at 5.    Cite R1.2.
    1.1 ≤ PF < 1.2:  cap score at 5.5.  Cite R1.2.
    1.2 ≤ PF < 1.3:  cap score at 6.    Cite R1.2.
    1.3 ≤ PF < 1.5:  cap score at 7.    Cite R1.2.
    PF ≥ 1.5:        no PF cap.
  When backtest n < 100, treat the displayed PF as the upper bound of true PF.
  BROAD_SCANNER alert type: R1.2 still applies if PF is reported. If PF is absent
  by design, R1.2 is a no-op and R1.4 applies.
...
// === VC MODEL CALL (v3 — resilience, retries, circuit breaker, fail-closed) ===
const _creds_VC = ($getWorkflowStaticData('global')._credentials) || {};
const VC_API_KEY = QTP_ANTHROPIC_KEY;
if (!VC_API_KEY) throw new Error('SE-C2: ANTHROPIC_API_KEY missing from $vars — failing closed');
...
if (!qtpAnthropicKeyLooksReal(VC_API_KEY)) {
  ...
  for (let attempt = 1; attempt <= VC_MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      console.error(`[VC CLAUDE] attempt ${attempt}/${VC_MAX_RETRIES} failed:`, vcGrokError);
...
if (vcGrokResponse) { await qtpMeterLLM(this.helpers, 'vc_gatekeeper', ticker, QTP_ANTHROPIC_MODEL, vcGrokResponse); }
if (!vcGrokResponse) {
...
// Pass through ALL signal data + the VC prompt + the VC model response
...
return [{
  json: {
    ...
    _vc_provider: 'anthropic_native',
    _vc_model: QTP_ANTHROPIC_MODEL,
    _vc_r32_policy: 'QTP_VC_R32_CONTRA_BOTH_v1_20260721',
