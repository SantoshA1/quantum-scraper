...
def compute(
    ...
    if qqq_close is not None:
        qqq_ema21 = ind.ema(qqq_close.reindex(ohlcv.index), 21)
        qqq_bias_bull = qqq_close.reindex(ohlcv.index) > qqq_ema21
        qqq_bias_bear = qqq_close.reindex(ohlcv.index) < qqq_ema21
    else:
        qqq_bias_bull = pd.Series(False, index=ohlcv.index)
        qqq_bias_bear = pd.Series(False, index=ohlcv.index)

    if spy_close is not None:
        spy_ema21 = ind.ema(spy_close.reindex(ohlcv.index), 21)
        spy_bias_bull = spy_close.reindex(ohlcv.index) > spy_ema21
        spy_bias_bear = spy_close.reindex(ohlcv.index) < spy_ema21
    else:
    ...
    equal_highs = (h - h.shift(1)).abs() <= eq_tol
    equal_lows  = (l - l.shift(1)).abs() <= eq_tol
    ...
    for i, (rv, alvt, avt) in enumerate(zip(
