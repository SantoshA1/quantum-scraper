...
def compute(ohlcv: pd.DataFrame, cfg: Optional[BridgeConfig] = None) -> pd.DataFrame:
    """
    Canonical module compute() entry point for scanner parity.

    Pine reference:
      * Technical block: AI Super Score Webhook Bridge v8 lines 28-37.
      * Sweep block: lines 90-115.

    Returns the joined technical + sweep DataFrame. ``cfg`` is optional and is
    used only for sweep parameters so existing callers can keep using
    compute_technicals() / detect_sweeps() directly.
    """
    cfg = cfg or BridgeConfig()
    return compute_technicals(ohlcv).join(
        detect_sweeps(ohlcv, cfg.sweep_lookback, cfg.sweep_vol_mult),
        how="left",
    )
...
def build_signal_payload(*args, **kwargs) -> dict:
    """
    Canonical scanner-facing payload builder.

    This keeps all indicator modules exposing a consistent payload-builder name
    while preserving the Webhook Bridge v8 field structure from Pine lines
    181-244. If the first argument is an OHLCV DataFrame, this delegates to
    build_from_bar(); otherwise it delegates to the low-level build_payload().
    """
    if args and isinstance(args[0], pd.DataFrame):
        return build_from_bar(*args, **kwargs)
