...
def parabolic_sar(
    ...
    # Wilder-style initialization: determine initial trend direction from
    # the first two bars. Pine's ta.sar uses this so the first SAR is the
    # opposite extreme of the initial trend.  Previously the code started
    # bull-by-default, causing PSAR drift in the first ~20 bars when the
    # series actually opens with a bearish move.
    af = start
    if h[1] >= h[0] and l[1] >= l[0]:
        is_bull = True
        ep = h[1]
        sar[0] = l[0]
    elif h[1] <= h[0] and l[1] <= l[0]:
        is_bull = False
        ep = l[1]
        sar[0] = h[0]
    else:
        # Mixed bar — fall back to original behaviour
        is_bull = True
        ep = h[0]
        sar[0] = l[0]
