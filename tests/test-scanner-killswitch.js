...
check('KS-07', 'margin cap still real under the proxy: $80k gross on $107k equity = 37.4% -> freezes', () => {
  const r = K.killSwitchDecision({ ...base, posCount: 8, grossExposure: 80000, alpacaInitialMargin: 0 }, 'proposed');
  assert.strictEqual(r.tripped, true);
  assert.ok(r.reason.startsWith('Margin 37%/35'), r.reason);
});
